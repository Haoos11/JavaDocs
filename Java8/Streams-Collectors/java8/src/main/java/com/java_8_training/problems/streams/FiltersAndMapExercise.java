...
public class FiltersAndMapExercise {
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter((String val) -> (val==name)).collect(toList());
        return personName;
    }
    ...
    public static List<Integer> filterNull(){
        ...
        evenNumbers = input.filter((Integer val)->(val!=null)).collect(toList());
        return evenNumbers;
    }
    ...
    public static List<Integer> flatteningLists() {
        ...
        together = input.flatMap(intr-> intr.stream()).collect(toList());
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min = 0;
        min = personList.stream().map(person -> (person.getAge())).reduce(Integer.MAX_VALUE, (x,y)->x<y?x:y);

        int min2 = 0;
        //min2 = personList.stream().collect(minBy(Person::getAge));
        return min;
    }
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream().filter(integer -> (integer%3)==0).findFirst();
    ...
    public static int sumStringLength(){
        ...
        //TODO #16
        //String::length
        sum = strings.stream().mapToInt(n -> n.length()).reduce(0,(acc,x)->acc+x);
